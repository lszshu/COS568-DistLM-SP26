# coding=utf-8
"""Task 3 distributed fine-tuning with DistributedDataParallel."""
...
def summarize_profiler(prof, args, train_history):
    trace_path = os.path.join(traces_dir(args), "task3_rank{}.json".format(rank_id(args)))
...
def train(args, train_dataset, model, tokenizer, ddp_state):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss.backward()
            sync_time = ddp_state.current_sync_time if ddp_state is not None else 0.0
            torch.nn.utils.clip_grad_norm_(model_for_optim.parameters(), args.max_grad_norm)
...
def main():
    ...
    logger.warning(
        "Process rank: %s, device: %s, distributed training: %s",
        args.local_rank,
        args.device,
    ...
    if args.do_train:
        ...
        global_step, tr_loss, train_history, profiler_summary = train(args, train_dataset, model, tokenizer, ddp_state)
