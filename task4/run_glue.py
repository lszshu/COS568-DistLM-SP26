# coding=utf-8
"""Task 4 profiling script for gather/scatter, all_reduce, and DDP."""
...
class DDPCommState(object):
    def __init__(self, process_group, world_size):
...
def timed_allreduce_hook(state, bucket):
    ...
    work = dist.all_reduce(bucket.buffer(), group=state.process_group, async_op=True)
    future = work.get_future()

    def _callback(fut):
        state.current_sync_time += time.perf_counter() - start_time
        tensor = fut.value()[0]
        return tensor / state.world_size

    return future.then(_callback)
...
def compute_profile_window(values):
    start_idx = 1
    end_idx = min(len(values), 4)
    return values[start_idx:end_idx]


def gather_scatter_sync(model, args):
    ...
    return time.perf_counter() - sync_start


def all_reduce_sync(model, args):
    sync_start = time.perf_counter()
    for param in model.parameters():
        if param.grad is None:
            continue
...
def save_deliverables(args, train_history, final_eval_results, profiler_summary):
    ...
    save_json(
        os.path.join(output_dir, "summary_rank{}.json".format(rank)),
        {
            ...
            "profiled_sync_overhead_pct": profiler_summary["profiled_sync_overhead_pct"],
            "epoch_eval_history": train_history["epoch_eval_history"],
            "final_eval_results": final_eval_results,
            "trace_file": profiler_summary["trace_file"],
            "communication_events": profiler_summary["communication_events"],
        },
    )
...
def summarize_profiler(prof, args, train_history):
    trace_path = os.path.join(
        traces_dir(args),
        "{}_rank{}.json".format(args.mode, rank_id(args)),
    )
    ...
    for event in prof.key_averages():
        key = event.key
        if any(keyword.lower() in key.lower() for keyword in comm_keywords):
    ...
    profiled_iteration_times = compute_profile_window(train_history["iteration_times_sec"])
    profiled_sync_times = compute_profile_window(train_history["sync_times_sec"])
    profiled_sync_overhead_pct = []
    for iteration_time, sync_time in zip(profiled_iteration_times, profiled_sync_times):
        if iteration_time > 0:
            profiled_sync_overhead_pct.append(100.0 * sync_time / iteration_time)
        else:
            profiled_sync_overhead_pct.append(None)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if ddp_state is not None:
                ddp_state.current_sync_time = 0.0
            ...
            if args.mode == "gather_scatter" and is_distributed(args):
                sync_time = gather_scatter_sync(model, args)
            elif args.mode == "all_reduce" and is_distributed(args):
                sync_time = all_reduce_sync(model, args)
            elif args.mode == "ddp" and ddp_state is not None:
                sync_time = ddp_state.current_sync_time

            model_for_clip = model.module if isinstance(model, DDP) else model
            ...
            if len(train_history["first_five_losses"]) < 5:
                ...
                logger.info(
                    "mode=%s rank=%d epoch=%d step=%d loss=%.6f",
                    args.mode,
                    rank_id(args),
                    epoch + 1,
                    step + 1,
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        suffix = prefix if prefix else "final"
        output_eval_file = os.path.join(eval_output_dir, "eval_results_{}.txt".format(suffix))
...
def main():
    ...
    parser.add_argument("--mode", type=str, required=True, choices=["gather_scatter", "all_reduce", "ddp"])
    ...
    os.makedirs(traces_dir(args), exist_ok=True)

    logging.basicConfig(
        format="%(asctime)s - %(levelname)s - %(name)s -   %(message)s",
        datefmt="%m/%d/%Y %H:%M:%S",
        level=logging.INFO if is_main_process(args) else logging.WARN,
    )
    logger.warning(
        "Process rank: %s, device: %s, distributed training: %s, mode: %s",
        args.local_rank,
        args.device,
    ...
    ddp_state = None
    if args.mode == "ddp" and is_distributed(args):
        ddp_state = DDPCommState(dist.group.WORLD, args.world_size)
        model = DDP(model)
        model.register_comm_hook(ddp_state, timed_allreduce_hook)
    ...
    if args.do_train:
        ...
        global_step, tr_loss, train_history, profiler_summary = train(
            args,
            train_dataset,
            model,
            tokenizer,
